...
const PairingResponse = require('../pairing-response')
...
const STATE = {
	WAITING_ON_REQUEST: 0,
	WAITING_ON_RESPONSE: 1,
	WAITING_ON_METHOD: 2,
  WAITING_ON_CONFIRM: 3,
  WAITING_ON_RANDOM: 4
};

var Smp = function(aclStream, localAddressType, localAddress, remoteAddressType, remoteAddress) {
  debug("Smp", JSON.stringify({localAddress: localAddress, remoteAddress:remoteAddress}));
...
Smp.prototype.sendPairingRequest = function() {
  ...
  this._pairingState = STATE.WAITING_ON_RESPONSE;

  this.write(this._preq);
};

Smp.prototype.sendCustomPairingRequest = function (customPairingRequest, passkeyOpt, passkeyVal) {
  debug("sendCustomPairingRequest", JSON.stringify(arguments));
...
Smp.prototype.onAclStreamEncrypt = function () {
  debug("onAclStreamEncrypt");
  this.emit('pairing', null, this._authType, this._assocModel)
}

Smp.prototype.handlePairingResponse = function (data) {
  debug("handlePairingResponse: ", JSON.stringify({data:data, waitingOnResponse:this._waitingOnResponse}));

  // so we can ignore unwanted pairing responses
  if (this._pairingState != STATE.WAITING_ON_RESPONSE){
    debug("Not waiting on Response... skipping...");
    return;
  }
  this._pairingState = STATE.WAITING_ON_METHOD;
  ...
  this._waitingOnMethod = true;
  this.emit('pairingResponse', null, new PairingResponse(this._pres));
}

/* BLUETOOTH SPECIFICATION Version 5.0 | Vol 3, Part H, Section 2.3.5.1 */
Smp.prototype.identifyAuthenticationMethod = function () {
  debug("identifyAuthenticationMethod");
  ...
  if (authType === SMP_AUTH_LEGACY) {
    ...
    // If neither device requires MITM protection, then use Just Works.
      ...
      debug('[SMP] Neither device requires MITM protection.')
...
Smp.prototype.parseIoCapabilities = function (reqIo, resIo, authType) {
  ...
  if (authType === SMP_AUTH_LEGACY) {
    ...
    if ((reqIo === SMP_IO_NOINPUTNOOUTPUT) || (resIo === SMP_IO_NOINPUTNOOUTPUT)) {
      debug('[SMP] Both devices are No Input No Output => Just Works.')
    ...
    // If both are Display Only, then Just Works is used.
      ...
      debug('[SMP] Both devices are Display Only => Just Works.')
      ioAssocModel = SMP_MODEL_JUSTWORKS
    } else if ((reqIo === SMP_IO_DISPLAYYESNO) || (resIo === SMP_IO_DISPLAYYESNO)) {
      debug('[SMP] At least one device is Display YesNo => Just Works.')
      ioAssocModel = SMP_MODEL_JUSTWORKS
    } else {
      debug('[SMP] IO capabilities for LE Legacy result in Passkey Entry.')
...
Smp.prototype.handleLegacyJustWorksPairing = function (data) {
  debug("handleLegacyJustWorksPairing");
  //this._pres = data;

  if (this._pairingState != STATE.WAITING_ON_METHOD ) {
    debug("Not waiting on method... Skipping...");
    return;
  }
  this._pairingState = STATE.WAITING_ON_CONFIRM;
...
Smp.prototype.handleLegacyPasskeyPairing = function (data) {
  debug("handleLegacyPasskeyPairing");
...
Smp.prototype.handleLegacyPasskeyPairingManual = function (data) {
  debug("handleLegacyPasskeyPairingManual");
  ...
  if (this._inputPasskey === null) {
    ...
    debug('Attempting pairing with passkey: ' + this._inputPasskey)
    this.handleLegacyPasskeyReceived(data)
  }
}

Smp.prototype.handleLegacyPasskeyReceived = function (data) {
  debug("handleLegacyPasskeyReceived");
...
Smp.prototype.handlePairingRandom = function(data) {
  ...
  if (this._pairingState != STATE.WAITING_ON_RANDOM) {
    debug("Not waiting on random... Skipping...");
    return;
  }
  this._pairingState = STATE.WAITING_ON_REQUEST;
...
Smp.prototype.handlePairingConfirm = function (data) {
  debug("handlePairingConfirm: " + data.toString("hex"));
  this._pcnf = data;

  if (this._pairingState != STATE.WAITING_ON_CONFIRM) {
    debug("Not waiting on confirm... Skipping...");
    return;
  }
  this._pairingState = STATE.WAITING_ON_RANDOM;
...
Smp.prototype.handleEncryptInfo = function(data) {
  debug("handleEncryptInfo: " + data.toString("hex"));
...
Smp.prototype.handleMasterIdent = function(data) {
  debug("handleMasterIdent: " + data.toString("hex"));
