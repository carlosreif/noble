...
Gap.prototype.onHciLeScanParametersSet = function() {
  debug("onHciLeScanParametersSet");
};

// Called when receive an event "Command Complete" for "LE Set Scan Enable"
Gap.prototype.onHciLeScanEnableSet = function(status) {
  debug("onHciLeScanEnableSet:",
  JSON.stringify({status:status, _scanState:this._scanState, _scanFilterDuplicates:this._scanFilterDuplicates}));
...
Gap.prototype.onLeScanEnableSetCmd = function(enable, filterDuplicates) {
  debug("onLeScanEnableSetCmd:", JSON.stringify({enable:enable, filterDuplicates:filterDuplicates}));
  ...
  if ((this._scanState == 'starting' || this._scanState == 'started')) {
    if (!enable) {
      debug('onLeScanEnableSetCmd: emit scanStop');
      this.emit('scanStop');
    } else if (this._scanFilterDuplicates !== filterDuplicates) {
      this._scanFilterDuplicates = filterDuplicates;
      debug('onLeScanEnableSetCmd: emit scanStart (FilterDuplicates changed)');
      this.emit('scanStart', this._scanFilterDuplicates);
    }
  } else if ((this._scanState == 'stopping' || this._scanState == 'stopped') && enable) {
    // Someone started scanning on us.
    debug('onLeScanEnableSetCmd: emit scanStart (Someone started scanning on us)');
    this.emit('scanStart', this._scanFilterDuplicates);
  }
};

Gap.prototype.onHciLeAdvertisingReport = function(status, type, address, addressType, eir, rssi, timestamp) {
  debug("onHciLeAdvertisingReport:", JSON.stringify({type:type}));
  ...
  // scanResponse
  if (type === 0x04) {
    advertisement.timestampResponse = timestamp;
    hasScanResponse = true;
    advertisement.hasScanResponse = true;
    advertisement.rawData = Buffer.concat([advertisement.scanData, eir]);
    advertisement.responseData = eir;
  } else {
    // reset service data every non-scan response event
    advertisement.hasScanResponse = false;
    advertisement.serviceData = [];
    advertisement.serviceUuids = [];
    advertisement.serviceSolicitationUuids = [];
    advertisement.rawData = eir;
    advertisement.scanData = eir;
    advertisement.responseData = undefined;
