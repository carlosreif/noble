...
var EVT_READ_REMOTE_VERSION_INFO = 0x0c;
...
var EVT_LE_READ_REMOTE_FEATURES_COMPLETE = 0x04;

var OGF_LINK_CTL = 0x01;
var OCF_DISCONNECT = 0x0006;
var OCF_READ_REMOTE_VERSION_INFO = 0x001D;
...
var OCF_LE_READ_REMOTE_FEATURES = 0x0016;

var DISCONNECT_CMD = OCF_DISCONNECT | OGF_LINK_CTL << 10;
var READ_REMOTE_VERSION_INFO_CMD = OCF_READ_REMOTE_VERSION_INFO | (OGF_LINK_CTL << 10);
...
var LE_READ_REMOTE_FEATURES_CMD = OCF_LE_READ_REMOTE_FEATURES | (OGF_LE_CTL << 10);
...
Hci.prototype.setSocketFilter = function() {
  debug("setSocketFilter");
  var filter = new Buffer(14);
  var typeMask = (1 << HCI_COMMAND_PKT) | (1 << HCI_EVENT_PKT) | (1 << HCI_ACLDATA_PKT);
  var eventMask1 = (1 << EVT_DISCONN_COMPLETE) | (1 << EVT_ENCRYPT_CHANGE) | (1 << EVT_CMD_COMPLETE) | (1 << EVT_CMD_STATUS) | (1 << EVT_READ_REMOTE_VERSION_INFO);
...
Hci.prototype.setEventMask = function() {
  ...
  var eventMask = new Buffer('ffffffffffffffff', 'hex');
...
Hci.prototype.setLeEventMask = function() {
  debug("setLeEventMask");
  var cmd = new Buffer(12);
  var leEventMask = new Buffer('ffffffffffffffff', 'hex');
...
Hci.prototype.readRemoteVersionInformation = function(handle) {
  debug("readRemoteVersionInformation");
  var cmd = new Buffer(6);

  // header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(READ_REMOTE_VERSION_INFO_CMD, 1);
  ...
  debug('Read Remote Version Information: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};

Hci.prototype.leReadRemoteFeatures = function(handle) {
  debug("leReadRemoteFeatures");
  var cmd = new Buffer(6);

  // header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(LE_READ_REMOTE_FEATURES_CMD, 1);
  ...
  debug('LE Read Remote Features: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
...
Hci.prototype.startLeEncryption = function(handle, random, diversifier, key) {
  ...
  // this command is result in a 'controller busy' status without the delay in a lot of cases
  setTimeout(function () {
    this._socket.write(cmd);
  }.bind(this), 100);
};
...
Hci.prototype.disconnect = function(handle, reason) {
  debug("disconnect: (" + handle + ") reason: " + reason);
...
Hci.prototype.onSocketData = function(data, timestamp) {
  debug('onSocketData: ' + 'data: ' + data.toString('hex') + ', ' + data.length + ' bytes');
  ...
  if (HCI_EVENT_PKT === eventType) {
    ...
    if (subEventType === EVT_DISCONN_COMPLETE) {
      ...
      this.processCmdCompleteEvent(cmd, status, result, handle);
    } else if (subEventType === EVT_READ_REMOTE_VERSION_INFO) {
      var len = data.readUInt8(2);
      var status = data.readUInt8(3);
      var handle = data.readUInt16LE(4);
      var version = data.readUInt8(6);
      var manufacturer_name = data.readUInt16LE(7);
      var subversion = data.readUInt16LE(9);
      debug('\t\tRead Remote Version Info ');
      debug('\t\tlen = ' + len);
      debug('\t\tstatus = ' + status);
      debug('\t\thandle = ' + handle);
      debug('\t\tversion = ' + version);
      debug('\t\tmanufacturer_name = ' + manufacturer_name);
      debug('\t\tsubversion = ' + subversion);
      this.emit('readRemoteVersionInfo', status, handle, version, manufacturer_name, subversion, data);
      ...
      this.processLeMetaEvent(leMetaEventType, leMetaEventStatus, leMetaEventData, timestamp);
    }
  } else if (HCI_ACLDATA_PKT === eventType) {
    debug("\tHCI_ACLDATA_PKT");
...
Hci.prototype.processCmdCompleteEvent = function(cmd, status, result, handle) {
  debug("\t\tprocessCmdCompleteEvent", JSON.stringify({cmd:cmd,status:status,result:result,handle:handle}));
  if (cmd === RESET_CMD) {
    this.setEventMask();
    this.setLeEventMask();
    this.readLocalVersion();
    ...
    this.hciVer = result.readUInt8(0);
    this.hciRev = result.readUInt16LE(1);
    this.lmpVer = result.readInt8(3);
    this.manufacturer = result.readUInt16LE(4);
    this.lmpSubVer = result.readUInt16LE(6);
    ...
    this.emit('readLocalVersion', this.hciVer, this.hciRev, this.lmpVer, this.manufacturer, this.lmpSubVer);
...
Hci.prototype.processLeMetaEvent = function(eventType, status, data, timestamp) {
  debug("\t\tprocessLeMetaEvent {eventType: " + eventType + ",  status:" + status + ", data:" + data.toString("hex") + "}");
  if (eventType === EVT_LE_CONN_COMPLETE) {
    debug("\t\t\tprocessLeMetaEvent EVT_LE_CONN_COMPLETE");
    this.processLeConnComplete(status, data);
  } else if (eventType === EVT_LE_ADVERTISING_REPORT) {
    debug("\t\t\tprocessLeMetaEvent EVT_LE_ADVERTISING_REPORT");
    this.processLeAdvertisingReport(status, data, timestamp);
  } else if (eventType === EVT_LE_CONN_UPDATE_COMPLETE) {
    debug("\t\t\tprocessLeMetaEvent EVT_LE_CONN_UPDATE_COMPLETE");
    this.processLeConnUpdateComplete(status, data);
  } else if (eventType === EVT_LE_READ_REMOTE_FEATURES_COMPLETE) {
    debug("\t\t\tprocessLeMetaEvent EVT_LE_READ_REMOTE_FEATURES_COMPLETE");
    this.processLeReadRemoteFeaturesComplete(status, data);
  }
  else {
    debug("\t\t\tprocessLeMetaEvent OTHER");
  }
};


Hci.prototype.processLeReadRemoteFeaturesComplete = function(status, data) {
  debug("processLeReadRemoteFeaturesComplete{status:" + status + ", data:" + data.toString("hex") + "}");
  var handle = data.readUInt16LE(0);
  var LeFeaturesRaw = data.slice(2);
  this.emit('LeReadRemoteFeaturesComplete', status, handle, LeFeaturesRaw);
};

Hci.prototype.processLeConnComplete = function(status, data) {
  debug("processLeConnComplete {status:" + status + ", data:" + data.toString("hex") + "}");
  var handle = data.readUInt16LE(0);
  ...
  this.emit('leConnComplete', status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy, data);
};

Hci.prototype.processLeAdvertisingReport = function(count, data, timestamp) {
  debug("processLeAdvertisingReport {count:" + count + ", data:" + data.toString('hex') + "}");
  for (var i = 0; i < count && data.length > 9; i++) {
    debug("processLeAdvertisingReport processing {i:" + i + ", data lenght: " + data.length + " data:" + data.toString('hex') + "}");
    ...
    this.emit('leAdvertisingReport', 0, type, address, addressType, eir, rssi, timestamp);
...
Hci.prototype.processLeConnUpdateComplete = function(status, data) {
  debug("processLeConnUpdateComplete {status:" + status + ", data:" + data.toString("hex") + "}");
  var handle = data.readUInt16LE(0);
...
Hci.prototype.processCmdStatusEvent = function(cmd, status) {
  debug("processCmdStatusEvent {cmd:" + cmd + ", status:" + status + "}");
  if (cmd === LE_CREATE_CONN_CMD) {
    if (status !== 0) {
      this.emit('leConnComplete', status);
    }
  }
  else if (cmd === READ_REMOTE_VERSION_INFO_CMD) {
